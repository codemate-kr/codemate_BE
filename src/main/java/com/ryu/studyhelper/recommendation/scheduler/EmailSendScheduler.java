...
/**
 * 이메일 발송 스케줄러
 * - 09:00: PENDING 상태 추천 발송
 * - 10:00: FAILED 상태 추천 1회 재발송
 */
@Component
...
public class EmailSendScheduler {
    ...
    public void retryFailedEmails() {
        ...
        try {
            result = recommendationEmailService.retryFailed();
            log.info("=== 이메일 재발송 배치 작업 완료 === (소요시간: {}ms)", System.currentTimeMillis() - startTime);
        } catch (Exception e) {
            failure = e;
            log.error("=== 이메일 재발송 배치 작업 실패 === (소요시간: {}ms)", System.currentTimeMillis() - startTime, e);
        }
    ...
    private void notifyRetry(BatchResult result, Exception failure, long elapsed) {
        try {
            if (failure != null) {
                discordNotifier.sendScheduler(DiscordMessage.error("이메일 재발송 배치 실패", failure, elapsed));
                return;
            }
            if (result.totalCount() == 0) {
                return;
            }
            String title = result.failCount() > 0 ? "이메일 재발송 배치 완료 (실패 있음)" : "이메일 재발송 배치 완료";
            discordNotifier.sendScheduler(DiscordMessage.batchResult(
                    title, result.totalCount(), result.successCount(), result.skipCount(), result.failCount(), elapsed));
        } catch (Exception e) {
            log.warn("Discord 알림 전송 실패", e);
